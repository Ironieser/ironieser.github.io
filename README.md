...
> **Branch note:** This is the `ironieser` personal-site branch, currently including the v2.1.0 roadmap layout (uniform tile grid, regrouped Agentic Systems stage) and page-view analytics. The clean reusable template remains on `master`; shared features are staged through `dev`. See [CHANGELOG.md](CHANGELOG.md) for details.
...
Version 2.0 requires the four-file config structure. The migration command converts
older single-file and root-level configs before the build runs.
