...
-- Each row is one deduplicated visit (same ip_hash within an hourly bucket counts once).
CREATE TABLE IF NOT EXISTS visits (
  id      INTEGER PRIMARY KEY AUTOINCREMENT,
  ts      INTEGER NOT NULL,   -- unix epoch seconds (UTC)
  bucket  INTEGER,            -- floor(ts/3600); (ip_hash, bucket) is UNIQUE for atomic de-dup
