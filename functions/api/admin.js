...
export async function onRequest({ request, env }) {
  ...
  try {
    ...
    const cities = await rows(
      "SELECT city, region, postal, country, COUNT(*) AS c FROM visits WHERE city IS NOT NULL AND city<>'' GROUP BY city, region, country ORDER BY c DESC LIMIT 50"
    );
    ...
    const orgs = await rows(
      "SELECT org, COUNT(*) AS c FROM visits WHERE org IS NOT NULL AND org<>'' GROUP BY org ORDER BY c DESC LIMIT 30"
    );
    const recent = await rows(
      "SELECT ts, country, city, region, postal, lat, lon, timezone, org, referer, path, substr(ip_hash,1,8) AS ip FROM visits ORDER BY ts DESC LIMIT 150"
    );
    ...
    return json({ total, unique, monthly, countries, cities, referers, orgs, recent });
  } catch (e) {
