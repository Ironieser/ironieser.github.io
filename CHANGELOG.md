...
- **Stage 03 regrouped into three layers** (`config/roadmap.yaml`) — 🧪 *Model Capabilities* (ToM, Dysarthric ASR, LogicIF, MLLM-Tool), ⚡ *Efficient Inference* (Rethinking Model Efficiency, MMTok), 🤖 *Long-Horizon Agents* (CineCrew, LiveMCP-101, Long-term Memory · Ongoing). New stage summary: "Building and evaluating model capabilities, running them efficiently, and composing them into agents that plan and act over long horizons." CineCrew and ASR TL;DRs rewritten to match.
